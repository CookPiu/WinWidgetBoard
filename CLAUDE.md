...
Two selectable network sources, one at a time (15-minute visible cadence, 10-second deadline), each with a readings endpoint and a search endpoint:

- **Open-Meteo** (`app.winwidgetboard.weather.open-meteo`) — the default, no account: `api.open-meteo.com` and `geocoding-api.open-meteo.com` (ADR 0027).
- **QWeather** (`app.winwidgetboard.weather.qweather`, ADR 0038) — the user's own API host under `*.qweatherapi.com`, added because the first source is not reachable from every network. One refresh issues three concurrent calls (`/weather/v1/current|hourly|daily`).

Location search is the only path that sends user-typed text off the machine, and only on explicit submit; it routes to whichever source is selected. Requests carry label, lat/long, and units — no account or device ID beyond the QWeather key. The location itself comes from Windows device location (one read per cold start in auto mode, first-run foreground consent, ADR 0032) or from manual search. Location label, coordinates, source selection, API host and the DPAPI-encrypted key persist to SQLite; **weather payloads never do** — they stay in the broker process, and failures degrade to Offline/Stale/Error while keeping the last in-process success (ADR 0020, 0021). Saving a location or switching source swaps the provider request key at runtime via `WeatherProviderRuntime` and publishes a Loading snapshot.

**A second source must not become a second card contract.** `QWeatherProvider` emits a payload identical in shape to Open-Meteo's — same field names, same WMO codes (`QWeatherConditionCodes` maps the vendor's codes into them), same metric numbers — so the panel, the taskbar entry and the snapshot pipeline cannot tell them apart. A vendor code with no sky meaning (heat/cold advisories, unknown) publishes *no* weather code rather than a nearby guess. Day/night comes from that location's own sunrise and sunset, because QWeather sends no is-day flag; timestamps are published as local wall clock with the offset stripped, or the panel re-renders a remote location's time in the machine's zone.

**The card's attribution is bound, not stated.** Both vendors require it, so the footer link takes its text from a resource key the projection derives from the payload's `source` and its URL from `attributionUrl`. A fixed `x:Uid` there credits whichever vendor was implemented first — which is exactly what it did until the second source shipped.
...
The project deliberately shrank to a "lightweight core" (ADR 0022), since re-approved up to seven capabilities: taskbar entry, panel, basic responsive layout, local notes, weather (device location + search, two selectable sources per ADR 0038), system monitor (ADR 0028), and token usage (ADR 0030). Timers, todos, clipboard, calendar, plugins, accounts, cloud sync, and telemetry are **deferred** — existing placeholder cards and base contracts stay as-is and must not be extended without explicit re-approval. Default rejections: new card types, new provider/plugin platforms, abstractions for a hypothetical second implementation, performance rewrites without measurements.
