...
便签 IPC 还必须验证方法、对象类型、字段长度、正文格式和 ISO-8601 时间戳；正文上限为 256 KiB，写操作使用当前用户命名管道、`clientOperationId` 有界幂等缓存和 revision 条件，不把便签正文写入日志或错误详情。WorkspacePanel 保存失败时只展示通用错误状态并保留内存草稿，不把正文写入诊断输出。

## 10. 网络

- 核心本地卡片不联网，联网仅由显式启用的 Provider 发起；
...
M2.3.9 的天气 Provider 只访问 `api.open-meteo.com`，请求只包含位置标签和经纬度，
不包含 Windows 账号、设备标识或自动定位结果；卡片显示
`Weather data by Open-Meteo.com` 来源链接。当前天气结果只在 CoreBroker 进程内按位置
保留最近成功 payload，不写入公共目录或 SQLite，断线时显示明确 Offline/Error 状态。
天气自动定位仍需要单独权限，尚未在本包启用。

M2.3.10 的天气位置设置通过当前用户 IPC 保存到用户数据库中的
`weather_settings` 表，仅包含用户输入的位置标签、经纬度、revision 和更新时间；不保存
天气响应、账号、设备标识或定位权限结果。隐私说明应展示 `api.open-meteo.com`、天气用途和
最近访问时间；用户清除天气设置时不等同于删除当前进程内的天气 payload，后者会在进程退出
时消失，独立的数据删除入口仍需后续工作包补齐。
...
| 图片缩略缓存 | 短 TTL、容量上限 |
| 天气位置设置 | 用户重置前 |
| 天气缓存 | 当前进程内最近有效响应、有限 TTL；不跨重启持久化 |
| 日志 | 滚动容量/天数上限 |
