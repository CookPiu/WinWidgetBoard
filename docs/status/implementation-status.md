...
M2.3.5 纯调度核心已收口。下一工作包应先冻结 CoreBroker Provider scheduler 生产宿主、`cards.subscribe` 结果适配和进程级不可恢复异常监督的边界，并评估 IPC/API/ADR 变更；在首发天气数据源、网络权限、缓存和隐私范围确认前，不接入真实 HTTP 或把内存 fake 描述为真实 Provider。以下保留累计路线与完成记录：
