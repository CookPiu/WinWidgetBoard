...
| 硬件监控 | 卡片与任务栏分段显示均已实现，显示项两处独立可配置并落盘；任务栏分段使用固定槽位，读数长度变化不再推动后续栏；公开 API 层覆盖 9 项读数（含 CPU 频率），采样按需进行，任务栏硬件胶囊启用时每秒采样并刷新一次；温度与风扇由 HWiNFO 共享内存补齐，未运行 HWiNFO 时报「需运行 HWiNFO」（[ADR-0033](../adr/0033-hwinfo-shared-memory-sensor-source.md)） | 本产品仍不分发任何内核驱动（[ADR-0029](../adr/0029-drop-the-bundled-sensor-driver.md)、[ADR-0031](../adr/0031-cpu-clock-from-per-core-counters.md)）；真实 HWiNFO 块的解析尚未在真机上验证 |
...
- **温度与风扇改由 HWiNFO 共享内存读取（2026-09-01）**。Before：这两项在任何机器上恒为
  ...
  **未验证**：参考机没有安装 HWiNFO，真实块的解析、选取策略在真实标签上的表现、以及有读数时的显示，
  ...
  另注：HWiNFO 免费版的共享内存有 12 小时限制，到期自动关闭需手动重开，届时卡片会回到「需运行 HWiNFO」；
...
- **传感器源文案已在生产路径实机验证**（2026-09-01，安装后构建，经任务栏入口打开面板）：
  硬件卡片经 UIA 播报 `CPU 温度 需运行 HWiNFO`，同一张卡上 `CPU 23%`、`内存 53% 16.6 GB / 31.4 GB`、
  `GPU 11%`、`CPU 频率 4.45 GHz`、`上传 1.8 KB/s`、`下载 2.5 KB/s` 均为真实读数。
  参考机没有安装 HWiNFO，**因此有传感器源时的读数与真实块的解析尚未在真机上验证过**；
