...
- UI 评审固定条件：浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、CoreBroker 断开占位数据、6 张卡片，基线提交 `96e7315`。Before：叠加层没有透视矩阵，`82°` 三维翻转被正交投影成压扁、`Offset.Z` 与 `Lift` 无画面效果，折痕/高光/遮板兜底色和圆角为写死常量，收起比展开慢约一倍，标题栏展开裁剪在动效结束后残留、会裁掉便签搜索结果，减少动态效果的淡化实际约 `1 s`；After：叠加层根视觉承载单台相机，折页有真实近大远小与纵深，颜色和圆角取自主题与源卡片，收起总时长 `549 → 460 ms`、展开 `700 → 583 ms`，裁剪在收敛时释放，减少动态效果为 `180 ms` 总时长；Why：让规范里承诺的透视深度真正成立，并让收起、无障碍降级和主题适配符合既有规范；
- Release UnitTests：396/396；
