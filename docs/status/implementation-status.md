...
| 硬件监控 | 卡片与任务栏分段显示均已实现，显示项两处独立可配置并落盘；任务栏分段使用固定槽位，读数长度变化不再推动后续栏；公开 API 层覆盖 9 项读数（含 CPU 频率），采样按需进行，任务栏硬件胶囊启用时每秒采样并刷新一次 | 温度与风扇恒为不可读，不再等待传感器层（[ADR-0029](../adr/0029-drop-the-bundled-sensor-driver.md)、[ADR-0031](../adr/0031-cpu-clock-from-per-core-counters.md)） |
| Token 用量 | 卡片已实现，读 Claude 与 Codex 的本机会话记录：两者格式与口径不同，Claude 按 `(requestId, messageId)` 去重、Codex 取会话累计量的相邻差分；分总览页与厂商页，设置页可启用/禁用厂商；以当日花费为主视觉，下列今日计费/缓存读取/响应数/缓存命中率/输出，各 token 类型带自己那份金额，金额按公开 API 价折算并标 `≈`，无价模型不计入并以 `+` 标为下限，价表带核验日期；24 小时趋势条；内容按卡片格数披露（S/M/W 两项读数，L 起全部）；卡片不可见时完全不读盘 | 不展示订阅配额（本地记录无法判断自身是否仍然成立，见 [ADR-0030](../adr/0030-token-usage-card.md) §2.2）；无请求级延迟；新增厂商的门禁见同一 ADR |
...
- **卡片缩放改为拖边框（2026-08-31，浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、生产数据，安装后实机手势验证）**。Before：进入编辑模式后每张卡片底部长出一条工具条（拖动柄 + 缩小 + 放大 + 移除），把「能做什么」写成文字，并且占掉卡片一行来放。After：卡片罩一层缩放边框——一圈细边加右边 / 下边 / 右下角三枚手柄，右上角一枚移除角标；八个卡片模板的工具条全部移除，`EditableCardSizes` 与两个步进按钮的点击处理随之删除。Why：**只有东南三个方向**，因为落位由确定性首次适配决定，从左或上缩放等于要求打包器尊重一个它随时会重排掉的原点；**吸附到五个尺寸**而不是自由像素，因为持久化的是 `sizeId`（ADR 0003 / 0014），指针挑列数与行数、落点吸到存在的那一档，平局吸窄的一档（拖过头是随后要纠正的动作）；**拖动只预览、松手才提交**，逐档提交会把一次手势压成三条撤销记录；没有 1 列 2 行这个尺寸，所以 `S` 拖高变 `L`。边框中间不接收指针输入，因此按住卡面仍是移动、卡内交互控件仍按原样隔离。两处首轮实机暴露的缺陷已修：**拖动过程没有预览**——`ApplyDropPreview` 会把被拖卡片钉在已提交位置（移动手势必须如此，卡片由变换带着走），缩放沿用它等于整个手势没有反馈，现另起 `ApplyResizePreview`，全部卡片都按投影落位；**指针没有反馈**——三枚手柄各自把光标换成 ↔ / ↕ / ⤡，离开换回箭头，`ProtectedCursor` 在 UIElement 上是 protected，因此边框改为真实控件类型 `CardResizeFrame`。证据：Release UnitTests 529/529（新增 `UT-GRID-060`～`066` 与 `UT-GRID-067` 覆盖吸附、平局、无效组合与轴隔离；`UT-GRID-039`/`047`、`UT-CARD-013`、`UT-UI-003` 四条结构契约同步改写到新标记）；`WorkspacePanel --smoke-test` 退出码 0；安装后实机以合成指针拖天气卡片下边手柄：卡片由 `L` 缩到 `M`（边框高 684 → 348 物理像素）、状态行报「卡片尺寸已更新」；**按住不放时卡片已经变成新尺寸**（边框高在按键未松开时即为 348），再拖回 `L` 后逐小时与三日预报重新出现；光标以 `GetCursorInfo` 逐点比对：卡面为箭头、下边为 `IDC_SIZENS`、右边为 `IDC_SIZEWE`、右下角为 `IDC_SIZENWSE`；移除角标点击真实删除卡片、撤销真实恢复。**未跑**：`Test-CardDragInteraction.ps1 -ResizeDragCombination` 等三个真实桌面脚本——它们的驱动点已由 `*CardDragHandle` 改指 `*CardResizeFrame`，但拖动类脚本需要交互式终端（Windows 不会把前台交给本来不在前台的控制台宿主），必须由使用者在自己的终端里跑一遍；
...
- **仍未跑**：`Test-CardDragInteraction.ps1` 的 `-WithBroker` 与 `-ResizeDragCombination`。四次尝试都止步于脚本自身的前置检查，原因已查清：脚本自启的面板抬不到最前——Windows 拒绝把前台交给一个本身不在前台的进程（控制台宿主永远不是），因此桌面上任何最大化窗口都会压住它。证据：报出的遮挡进程随当前开着的窗口变化（`REDAgent` → `claude` → 把 `claude` 最小化后变成 Progman 桌面），而面板几何与任务栏并不重叠（面板底边 `1884`，任务栏 `1904` 起）。**由使用者在自己的交互式终端里运行即可通过**——那时控制台就是前台进程。已尝试在共享模块里用一次真实点击换取前台权限，未能解决，改动已回退，不留未验证的脚本改动；
...
- 真实桌面 `Test-SystemMonitorInteraction.ps1` 通过（`REAL-SYSMON-PASS`）：卡片经 UIA 播报
  `内存 63% 19.8 GB / 31.4 GB`，在对话框中切换 `cpu.clock` 后卡片在一个刷新周期内跟随，
  播报 `CPU 频率 4.37 GHz`（脚本现在断言该行必须给出 GHz/MHz 读数，仅有行名视为失败），
  重启面板后设置从 SQLite 读回；
...
温度与风扇恒显示「此电脑无法读取」；CPU 频率并不需要驱动，已按
[ADR-0031](../adr/0031-cpu-clock-from-per-core-counters.md) 改为可读。
