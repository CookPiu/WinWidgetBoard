...
| Token 用量 | 卡片已实现：读本机会话转录，按 `(requestId, messageId)` 去重，今日计费/输出/缓存读取/响应数、缓存命中率、当前与峰值速率（同为 15 分钟窗口）及 24 小时趋势条；卡片不可见时完全不读盘 | 无费用估算与请求级延迟，两者的门禁见 [ADR-0030](../adr/0030-token-usage-card.md) |
