...
| 天气 | 已实现 Open-Meteo、订阅、Windows 自动位置、手动位置和重启恢复设置；首次前台授权后每次冷启动读取一次系统位置，失败保留手动位置；面板关闭时按小时后台刷新，任务栏入口可经 `weather.summary.get` 显示读数 | 跨重启 payload 缓存延期 |
