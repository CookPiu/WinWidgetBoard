...
- 真实桌面 `Test-WeatherSettingsInteraction.ps1` 连续 3 次完整通过：`REAL-WEATHER-SETTINGS-PASS`（保存经真实面板写入 SQLite 并触发 provider 运行时重载）、`REAL-WEATHER-SETTINGS-RESTART-PASS`（重启后 `weather.settings.get` 读回 Tokyo / 35.6762 / 139.6503）和 `WINDOW-EXIT-PASS exitCode=0`；
...
5. 面板在模态对话框关闭后会静默丢弃第一次关闭点击：`RequestCloseMotion` 在 `_modalScopeDepth != 0` 时直接返回，而该作用域要到设置对话框处理函数写完状态文本才释放。已在参考机稳定复现（第二次点击必定生效），是否改为释放后补发或排队关闭请求需要单独决定。
6. Windows App SDK 自包含输出较大，开发构建不应长期留在仓库。
7. 干净网络 restore、CI、完整显示/无障碍/性能和发布矩阵尚未完成。
8. 计时器、待办和日历以延期占位卡保留在默认工作区，已确认维持现状；它们只作为布局占位，不增加业务行为，也不再作为待决问题。
...
- 天气设置真实流程改为只在面板进程自己的顶层窗口内查找元素，不再遍历桌面根；共享模块对可重试的 UIA COM 故障退避重试，该流程首次完整跑通并连续 3 次通过。

## 7. 下一步

按单一目的拆分：

1. 决定模态对话框关闭后被丢弃的首次关闭点击如何处理（见 §5.5）；
