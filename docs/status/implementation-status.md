...
- **卡片缩放改为拖边框（2026-08-31，浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、生产数据，安装后实机手势验证）**。Before：进入编辑模式后每张卡片底部长出一条工具条（拖动柄 + 缩小 + 放大 + 移除），把「能做什么」写成文字，并且占掉卡片一行来放。After：卡片罩一层缩放边框——一圈细边加右边 / 下边 / 右下角三枚手柄，右上角一枚移除角标；八个卡片模板的工具条全部移除，`EditableCardSizes` 与两个步进按钮的点击处理随之删除。Why：**只有东南三个方向**，因为落位由确定性首次适配决定，从左或上缩放等于要求打包器尊重一个它随时会重排掉的原点；**吸附到五个尺寸**而不是自由像素，因为持久化的是 `sizeId`（ADR 0003 / 0014），指针挑列数与行数、落点吸到存在的那一档，平局吸窄的一档（拖过头是随后要纠正的动作）；**拖动只预览、松手才提交**，逐档提交会把一次手势压成三条撤销记录；没有 1 列 2 行这个尺寸，所以 `S` 拖高变 `L`。边框中间不接收指针输入，因此按住卡面仍是移动、卡内交互控件仍按原样隔离。证据：Release UnitTests 528/528（新增 `UT-GRID-060`～`066` 覆盖吸附、平局、无效组合与轴隔离；`UT-GRID-039`/`047`、`UT-CARD-013`、`UT-UI-003` 四条结构契约同步改写到新标记）；`WorkspacePanel --smoke-test` 退出码 0；安装后实机以合成指针拖天气卡片下边手柄：卡片由 `L` 缩到 `M`（边框高 684 → 348 物理像素）、状态行报「卡片尺寸已更新」，再拖回 `L` 后逐小时与三日预报重新出现；移除角标点击真实删除卡片、撤销真实恢复。**未跑**：`Test-CardDragInteraction.ps1 -ResizeDragCombination` 等三个真实桌面脚本——它们的驱动点已由 `*CardDragHandle` 改指 `*CardResizeFrame`，但拖动类脚本需要交互式终端（Windows 不会把前台交给本来不在前台的控制台宿主），必须由使用者在自己的终端里跑一遍；
