...
| 硬件监控 | 卡片与任务栏分段显示均已实现，显示项两处独立可配置并落盘；公开 API 层覆盖 8 项读数，采样按需进行，任务栏硬件胶囊启用时每秒采样并刷新一次 | 温度、风扇、CPU 频率恒为不可读，不再等待传感器层（[ADR-0029](../adr/0029-drop-the-bundled-sensor-driver.md)） |
