...
- **Token 定价每日同步（2026-09-02，[ADR-0035](../adr/0035-daily-token-pricing-sync.md)）**。Before：用户当天把
  ...
  证据：Release UnitTests 565/565（新增 `UT-TOKUSE-110`～`115`：一方 id 过滤、缺失缓存字段按厂商倍率补、
  同步价优先、节奏、落盘往返、内置表）；安装后生产库 `user_version = 9`，`token_usage_pricing` 一行
  ...
  「价表更新于 2026/9/2 9:47（来源：LiteLLM 开源价表）」。**未验证**：关闭开关后不再访问端点，以及
  304 路径，都只由单元测试与代码路径覆盖，未抓包；
