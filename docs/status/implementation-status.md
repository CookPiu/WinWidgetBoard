...
- UI 评审固定条件：浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、生产数据（北京）、6 张卡片，基线提交 `6c5ae22`。Before：卡片只有实况，且体感/湿度/风速/观测时间是同一权重的四行 caption，观测时间与气温平起平坐；位置名称在没有读数时回退到硬编码英文 `"Weather"`，读起来像一个地名；观测时间原样显示 provider 的 ISO 串。After：provider 扩展 Open-Meteo 请求，同一端点多取 `hourly` 与 `daily`；卡片改为「现在 / 今天接下来 / 之后几天」三段，逐小时横向滚动，每日高低温分列；条件字形由圆和矩形拼出（云占上三分之二、降水记号占下三分之一，共用同一条带会让 18 DIP 的雨糊成黑块）；无读数时不显示位置行；观测时间格式化为时钟读数。Why：预报是这张卡片能给出「有逻辑的信息」的唯一杠杆——只有五个实况数值时，层级再排也到此为止；
