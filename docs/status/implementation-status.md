...
- [x] 已按 [M2.3.9 Open-Meteo 天气 Provider 垂直切片工作包](../work-packages/M2.3.9-open-meteo-weather-provider.md) 接入真实 `weather.current`：Provider 以 15 分钟可见刷新接入唯一 Scheduler，`ProviderRefreshVisibilityRegistry` 将 `cards.subscribe` 面板/视口/断线状态反馈到 Provider，Weather runtime 显示当前温度、体感、湿度、风速、观测时间和来源链接；默认位置为 Singapore，当前进程内保留同一位置最近成功 payload。Debug/Release UnitTests 285/285，CoreBroker/WorkspacePanel Debug/Release x64 构建 0 警告、0 错误，Release smoke 退出码 0，五状态锚点 UIA 与真实 Open-Meteo CoreBroker→cards.snapshot→WorkspacePanel 链路通过；自动定位、设置持久化、跨重启缓存和 OS 网络/电源事件仍不在本包；
- [x] 已按 [M2.3.10 天气位置设置工作包](../work-packages/M2.3.10-weather-location-settings.md) 接入双语天气位置设置对话框、`CardSettingsDraft` 提交门禁、`weather.settings.get/save` IPC、SQLite schema v3/仓储与 revision 冲突保护，以及保存后的 Provider 注册替换和可见性继承；Debug/Release UnitTests 均为 292/292，CoreBroker/WorkspacePanel/UnitTests Debug/Release x64 构建均为 0 警告、0 错误；真实 UIA 输出 `REAL-WEATHER-SETTINGS-PASS`、`WEATHER-SETTINGS-RESTART-PASS`、`WINDOW-EXIT-PASS`，并完成 M2.3.9 Open-Meteo 真实天气回归；自动定位、跨重启天气 payload 缓存和 OS 网络/电源事件仍不在本包；
...
M2.3.10 已完成天气位置设置 UI/IPC/SQLite 持久化、Provider 运行时切换和真实桌面验证；下一步优先处理自动定位、跨重启天气 payload 缓存和 OS 网络/电源事件。以下保留累计路线与完成记录：
...
51. [x] 已按 [M2.3.9 Open-Meteo 天气 Provider 垂直切片工作包](../work-packages/M2.3.9-open-meteo-weather-provider.md) 接入真实 `weather.current`、Provider 可见性反馈、Weather runtime/来源链接和进程内缓存回退；Debug/Release UnitTests 285/285、CoreBroker/WorkspacePanel Debug/Release 构建、Release smoke、五状态锚点 UIA、真实 HTTPS 和真实 CoreBroker→cards.snapshot→WorkspacePanel 链路均已通过；自动定位、设置 UI/IPC/持久化、跨重启缓存和 OS 网络/电源事件仍待后续。
52. [x] 已按 [M2.3.10 天气位置设置工作包](../work-packages/M2.3.10-weather-location-settings.md) 接入天气设置 UI/IPC/SQLite 持久化、revision 冲突保护和 Provider 运行时切换；Debug/Release UnitTests 292/292、CoreBroker/WorkspacePanel/UnitTests Debug/Release x64 构建、真实保存/重启读取/正常退出 UIA 和 M2.3.9 天气回归均已通过；自动定位、跨重启天气 payload 缓存和 OS 网络/电源事件仍待后续。
