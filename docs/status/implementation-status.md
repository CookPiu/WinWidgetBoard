...
M2.3.9 已完成 Open-Meteo 真实 Provider、Weather runtime 和受控 HTTPS/桌面验证；下一步优先补设置 UI/IPC/持久化，随后再处理自动定位、跨重启天气缓存和 OS 网络/电源事件。以下保留累计路线与完成记录：
