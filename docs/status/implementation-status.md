...
7. 计时器、待办和日历以延期占位卡保留在默认工作区，已确认维持现状；它们只作为布局占位，不增加业务行为，也不再作为待决问题。
...
1. 判断面板首帧约 670 ms、布局就绪约 880 ms 是否需要优化，若需要则先定位耗时构成再改；
2. 继续拆分 `MainWindow.xaml.cs` 中便签删除/编辑、拖动和设置的协调职责；
3. 仅修复核心五项的可复现问题。
