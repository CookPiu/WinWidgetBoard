...
`M2.2 — 网格布局（M1.0 环境门禁和完整显示器矩阵仍待完成；M1.1～M1.3 当前会话验收已完成，M2.0.1～M2.0.3、M2.1.1～M2.1.6、M2.2.1～M2.2.12 和 M2.4.1 已完成当前工作包；便签完整能力、其他领域 CRUD、剩余布局/卡片业务与活动数据库恢复演练待完成）`
...
- [x] M2.4.1 已为 NTE-001 便签卡片加入显式复制按钮，复制标题和正文原文到 Windows 当前剪贴板，不写入应用数据库；`UT-NOTE-010/011` 覆盖格式化和空段处理，Release UnitTests 113/113，WorkspacePanel Release x64 构建 0 警告、0 错误，WinUI smoke 返回 0；真实桌面剪贴板粘贴回归仍待完成；
...
31. [x] 已按 [M2.4.1 NTE-001 便签显式复制动作工作包](../work-packages/M2.4.1-note-copy-action.md) 完成复制格式化器、双语按钮和剪贴板写入；下一步补便签搜索/恢复、待办与显示矩阵。
