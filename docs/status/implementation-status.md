...
- UI 评审固定条件：浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、隔离测试数据、6 张卡片，基线提交 `099d42e`。Before：设置浮层的按钮套用 `WwbToolbarButtonStyle` 系列，而那两个样式把字体设为 `Segoe Fluent Icons`（内容本该是单个字形），「搜索」「保存」渲染成豆腐块——硬件监控的保存按钮在合并之前就已经是这个样子；天气位置只能靠搜索，没有可直接选的清单；保存按钮排在分类内容末尾，内容一长就滚出视野。After：新增 `WwbDialogButtonStyle` / `WwbPrimaryDialogButtonStyle` 两个文字按钮样式并全部改用；天气分类在未搜索时列出 34 个常用地点，搜索后由结果列表取代，两份列表共用同一行模板；每个分类拆为「滚动主体 + 固定提交区」，输入、保存与状态常驻可见；空的校验行折叠而不是留白；主体与提交区共用一条左边线。Why：这一轮把三个同源缺陷一并按掉——`ContentDialog` 内的内容按无界宽度测量，星号列会把整个宽度交给第一个子元素、把 `Auto` 列排到裁剪之外，元素仍在自动化树里但矩形为空。天气搜索按钮、硬件监控的下移按钮都是这样消失的。本浮层因此改为不使用星号列，宽度一律显式写出，规范 3.3 已记录；
- Release UnitTests：403/403；
