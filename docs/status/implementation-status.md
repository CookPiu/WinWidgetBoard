...
`M2.2 — 网格布局（M1.0 环境门禁和完整显示器矩阵仍待完成；M1.1～M1.3 当前会话验收已完成，M2.0.1～M2.0.3、M2.1.1～M2.1.6 和 M2.2.1～M2.2.11 已完成当前工作包；便签完整能力、其他领域 CRUD、剩余布局/卡片业务、撤销/重做与活动数据库恢复演练待完成）`
...
- [ ] M2.1 便签完整能力、其他领域 CRUD、M2.2 剩余布局/卡片业务、20 步撤销/重做和活动数据库恢复；
- [ ] CoreBroker 与 IPC。
- [ ] 内置卡片。
- [ ] 安装、更新与签名。
