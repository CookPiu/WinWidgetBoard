...
| Token 用量 | 卡片已实现，支持 Claude 与 Codex 两家：两者记录格式与口径不同，Claude 按 `(requestId, messageId)` 去重、Codex 取会话累计量的相邻差分；卡片分总览页与厂商页，设置页可启用/禁用厂商；今日计费/输出/缓存读取/响应数、缓存命中率、当前与峰值速率（同为 15 分钟窗口且同为今日）、24 小时趋势条；Codex 页另显示其自报配额，重置时间已过的窗口不显示；内容按卡片格数披露——读数按优先级截断（S/M/W 两项、L 四项、XL 全部），趋势需两行，四列宽时占比与配额移到侧栏；今日计费旁显示按公开 API 价折算的估算金额（`≈` 前缀，无价模型不计入并以 `+` 标为下限，价表带核验日期）；卡片不可见时完全不读盘 | 无费用估算与请求级延迟，新增厂商的门禁见 [ADR-0030](../adr/0030-token-usage-card.md) |
