...
- UI 评审固定条件：浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、生产数据、6 张卡片，基线提交 `72ebe86`。Before：便签正文写死 `148 DIP`，卡片放大只多出空白，可写区域纹丝不动。After：编辑器随卡片长高——滚动内容的 `MinHeight` 绑定到滚动器 `ViewportHeight`，正文占星号行、`MinHeight 120`、垂直拉伸；Markdown 预览的上限同步由 `148` 提到 `320`。Why：这张卡片存在的理由就是那块可写区域，放大买到的却是它周围的留白。星号行在 `ScrollViewer` 里被按无界高度测量并塌缩回内容高度，与设置浮层「不用星号列」是同一个测量陷阱的两面；`UT-UI-005` 原先把 `Height=148` 钉成契约，已改为更强的断言（不得有固定高度、必须有 `MinHeight` 与拉伸、滚动内容必须绑定视口高度）；
