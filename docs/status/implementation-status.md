...
- UI 评审固定条件：浅色主题、`3200×2000 @ 165 Hz`、200% DPI、中文、隔离测试数据、6 张卡片，基线提交 `aab3a2a`。Before：网速固定为「全部可用网卡合计」，装了 VMware 或挂着 VPN 的机器无法只看真正在用的那块。After：硬件监控分类新增「网络来源」下拉，第一项恒为「全部网卡（合计）」，其余由 Broker 实时枚举，与采样器共用同一套过滤条件。Why：这项与其余显示项不同——它改变**采什么**而不是**显示什么**，所以保存后直接下推到采样器并丢弃速率基线（跨越两组不同计数器的差值描述不了任何真实时刻），同时清空任务栏火花线的历史窗口，否则那条线会把两个来源画成一条连续曲线；
- Release UnitTests：407/407；
