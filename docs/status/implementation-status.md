...
常驻面板实测代价：工作集 248.4 MB、私有内存 208.1 MB、空闲 CPU 94 ms/30 秒（全核 0.022%）。收益是入口点击到面板由约 740 ms 降到约 20～110 ms。是否用约 250 MB 常驻内存换这段延迟，待产品决定。
...
4. 面板冷启动耗时构成与常驻代价均已测量（见 §4.2）：低成本优化没有明显空间，常驻可把开启延迟压到约 1/7，代价是约 250 MB 常驻内存。等产品决定。
