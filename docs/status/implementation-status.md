...
`M2.2 / M2.4 当前代码范围与 M2.3.0～M2.3.4 已收口（M2.2.13 / M2.4.11 已完成当前单显示器真实桌面验收、验收进程/数据隔离和自动回归；M2.3.1 已完成 Definition/Instance、生命周期、不可变快照、订阅和实例级错误边界；M2.3.2 已完成面板/ItemsRepeater 实现视口驱动的 Hidden/Visible、集中式本地快照调度、取消和迟到结果门禁；M2.3.3 已完成 UI 无关的设置草稿、预览取消恢复和提交门禁；M2.3.4 已完成十态纯投影、五模板共享状态区、双语资源、静态加载骨架和未建模动作门禁；设置 UI/IPC/持久化、真实 Provider 周期/退避、M1.0 干净环境与 CI、真实多显示器/DPI/完整无障碍与性能矩阵、便签版本历史、删除撤销、其他领域 CRUD、剩余卡片业务和正式数据库恢复演练仍待完成）`
...
M2.3.4 已收口。下一工作包应先冻结 M2.3.5 真实 Provider 刷新、退避与节能策略的最小合同，并继续与设置 UI/持久化、网络/IPC 协议扩展和具体卡片业务分离；在工作包与验收边界批准前不把测试快照描述为真实 Provider。以下保留累计路线与完成记录：
