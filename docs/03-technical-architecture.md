...
用量读自本机会话转录 `%USERPROFILE%\.claude\projects\*\*.jsonl`，
不接任何计费 API，因此 provider 的 `requiresNetwork` 为 false
（[ADR-0030](adr/0030-token-usage-card.md)）。

两条不变量决定了实现形状：

- **同一次响应在转录里占多行**，每个 content block 一行且各自重复整个 usage 对象。
  按 `(requestId, messageId)` 去重是正确性要求——不去重会让用量虚高近一倍；
- **转录没有耗时字段**，因此只提供墙钟窗口速率，不提供单次响应速度。
  当前速率与峰值速率使用**相同的 15 分钟窗口**，否则两个分母会让"当前"读数远高于"峰值"。

读取是增量的：每个文件保留字节水位，只读新增部分，且水位只推进到最后一个完整行之后——
转录正被另一进程写入，半行必须留给下一次。文件变短视为截断或重建并整份重读，由去重兜住。

转录含完整对话，所以解析用 `Utf8JsonReader` 按子树跳过 `content`，消息正文不构造字符串；
