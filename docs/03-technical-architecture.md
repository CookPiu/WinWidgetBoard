...
- 不引用 WinUI；
- 验证全部 IPC 输入；
- 持有数据库、全部网络访问（当前天气数据源的天气读数与地点搜索两个端点，以及每日一次、可关闭的
  LiteLLM 价表拉取，[ADR-0035](adr/0035-daily-token-pricing-sync.md)）和集中调度；
- 天气数据源由设置选择，两家实现向卡片发布**完全同形**的载荷——同样的字段名、同样的 WMO
  天气代码、同样的公制数值，厂商差异全部吸收在 provider 内部，面板与入口分辨不出来源
  （[ADR-0038](adr/0038-second-weather-provider-qweather.md)）；
