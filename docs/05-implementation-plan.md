...
前置视觉工作包 [M2.3.0 UI 基础样式与共享卡片外壳](work-packages/M2.3.0-ui-visual-foundation.md)
已完成：建立四像素间距、系统主题画刷、共享卡片/输入/命令样式，并通过当前
DPI 192 真实窗口、四卡拖拽、布局历史和便签 UIA 回归。该工作包不包含下列
运行时状态、生命周期、订阅或调度能力。
