...
卡片经 `cards.snapshot` 收到的每个读数，除已排版的 `primaryText`、`secondaryText` 与 `ratio`
之外还带最近窗口：

- `history`：由旧到新、已归一化到 0..1 的最近采样，最多 60 个点，与 `sysmon.summary.get` 的同名
  字段**由同一段代码按同一套刻度产生**——有上限的按上限、速率按窗口自身峰值；不足两个点时为空数组，
  卡片此时退回仪表条；
- `historyTexts`：与 `history` 等长的已排版文本，**只有卡片能显示的第一个读数**带它。只有那一条被
  画成首屏大数、接得住指针，其余曲线是行的衬底；每条都带等于每两秒重复排版八条谁也悬停不到的序列。
  它存在的理由是归一化之后还原不出数值：速率的 0..1 相对于面板从来看不到的峰值；
- 载荷级的 `sampleIntervalSeconds`：相邻两点相隔多少秒。十字线要说出「多久以前」，而序列本身不带
  时间戳；面板自行假定节奏，会在 Broker 改节奏的那天开始说谎。为 0 或缺失时十字线只报数值。

等待传感器来源的读数（`needs-sensor-source`）不上卡片，因此也不会成为那个「第一个读数」。
...
原始数值。没有刻度可画或本机读不到的指标（温度、风扇、CPU 频率）返回空数组；
空闲网卡返回真实的一串 0，两者由入口区别对待。
