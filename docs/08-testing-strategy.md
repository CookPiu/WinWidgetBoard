...
- 临时用户数据目录；
- 写入型桌面验收必须显式使用 `--acceptance-test`、
  `--test-instance-id <GUID>` 和系统临时目录下的 `--data-directory`；
- 不得把修改 `LOCALAPPDATA` 视为数据库隔离证据；
- 脚本只终止自己启动并持有句柄的进程，递归清理前必须验证目标仍在系统临时目录内；
- 写入卡片、布局、便签和计时器；
- 杀死进程；
- 重启恢复；
- 通知计划；
...
- 便签编辑撤销与重做；
- 便签 Markdown/纯文本模式切换；
- 便签基础 Markdown 预览与源文往返；
- 多便签列表、新建、搜索打开、草稿保护和删除；
- 便签保存重试、已提交状态恢复和 Broker/Panel 重启；
- 缩放；
- 新增便签；
- 新增待办；
- 启动计时器；
...
当前受控回归已在 Release x64 下通过便签、计时器、待办和本地日历的拖动柄与
非交互卡片表面，验证卡片内交互控件隔离和 `Esc` 取消；布局历史同时通过按钮与
`Ctrl+Z`/`Ctrl+Y`。隔离 `-WithBroker` 路径已完成“拖动提交—Broker/Panel
重启—再次编辑”，并在同一物理显示器上用受控启动几何验证 2/4/6 列视觉分布。
该证据不替代真实系统 DPI 切换、多显示器、屏幕阅读器或刷新率帧时间矩阵。
