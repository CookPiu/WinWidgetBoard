...
M2.2.1 已完成响应式网格纯函数、尺寸映射和确定性无重叠排布测试，M2.2.2 已加入可测试的卡片布局 ViewModel，M2.2.3 已接入 ItemsRepeater 卡片视觉排布，M2.2.4 已加入显式编辑模式门禁，M2.2.5 已加入确定性拖动落点投影，M2.2.6 已接入指针拖动和释放提交，M2.2.7 已修复拖动边界和视觉变换回收问题，M2.2.8 已接入 CoreBroker/SQLite 布局持久化，M2.2.9 已修复便签状态覆盖和逻辑网格位置重放，M2.2.10 已统一四种卡片的拖动入口并接入占用单元格的实时让位预览，M2.2.11 已恢复异常大的持久化空行，并修复异步加载后的模板身份和提交后网格重排，M2.2.12 已加入布局编辑会话 20 步撤销/重做；M2.4.1 已加入便签显式复制动作，M2.4.2 已加入保存失败后的显式重试并保留内存草稿，M2.4.3 已加入便签搜索结果展示，M2.4.4 已加入从搜索结果安全打开便签，M2.4.5 已加入便签编辑撤销/重做，M2.4.6 已加入 Markdown/纯文本模式和基础 Markdown 预览，M2.4.7 已加入最后提交状态恢复回归，M2.4.8 已加入多便签只读列表与安全导航，M2.4.9 已加入新建便签并安全切换；让位弹簧补间、虚拟化回收和完整可访问性验收仍在后续工作包中。

M2.4.10 已加入多便签安全编辑与删除：列表项使用 revision 删除保护，删除前二次确认，删除失败保留当前内容；真实桌面 UIA 回归仍待运行。
